...
const moduleConfig = {
  home: { label: "Landing Architecture", allowCreate: true, desc: "Global hero framework and initial landing professional standards." },
  about: { label: "Professional Profile", allowCreate: true, desc: "Technical background, core mission, and mission assets." },
  services: { label: "Service Catalog", allowCreate: true, desc: "Engineering packages and architectural consultancy definitions." },
  projects: { label: "Project Registry", allowCreate: true, desc: "High-performance case studies and structural delivery logs." },
  testimonials: { label: "Client Heritage", allowCreate: true, desc: "Industry-standard feedback and professional client reviews." },
  gallery: { label: "Visual Archive", allowCreate: true, desc: "Structural renders and project photography laboratory." },
  contactDetails: { label: "Contact Infrastructure", allowCreate: true, desc: "Communication hubs and geographic coordination data." },
  seoMeta: { label: "Search Visibility", allowCreate: true, desc: "Global search presence and metadata optimization standards." },
  contactSubmissions: { label: "Client Inquiries", allowCreate: false, desc: "Professional project inquiries and communication logs." },
};
...
export default function AdminContentPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (!ignore) {
          setStatus("LOAD_ERROR: RE-LOGIN REQUIRED");
        }
      } finally {
        if (!ignore) {
          setLoading(false);
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      if (selectedId) {
        ...
        setStatus("COMMIT_SUCCESS: UPDATE_APPLIED");
      } else {
        await adminCreate(type, payload);
        setStatus("COMMIT_SUCCESS: NEW_RECORD_INITIALIZED");
      }
      await reload();
      setSelectedId("");
      setForm(baseForm);
    } catch {
      setStatus("COMMIT_FAILED: VALIDATION_ERROR");
    } finally {
      setSaving(false);
    }
  };
  ...
  const onDeleteItem = async (itemId) => {
    ...
    try {
      ...
      setStatus("COMMIT_SUCCESS: RECORD_DESTROYED");
      await reload();
      ...
      setStatus("COMMIT_FAILED: PERMISSION_DENIED");
    } finally {
      setSaving(false);
    }
  };
  ...
  const onStatusUpdate = async (itemId, nextStatus) => {
    try {
      ...
      await reload();
      setStatus("STATUS_UPDATE: " + nextStatus.toUpperCase());
    } catch {
      setStatus("STATUS_UPDATE: FAILED");
    }
  };
  ...
  return (
    <div ref={containerRef} className="mx-auto mt-8 md:mt-12 max-w-[1600px] px-4 md:px-8 pb-32">
      <header className="admin-reveal mb-8 md:mb-16 flex flex-col md:flex-row items-start md:items-center justify-between gap-10 md:gap-12 rounded-[32px] md:rounded-[56px] border border-white/5 bg-[#080808]/80 p-8 md:p-20 shadow-[0_80px_160px_rgba(0,0,0,0.8)] backdrop-blur-3xl">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-10">
          <div className="h-16 w-16 md:h-20 md:w-20 rounded-[24px] md:rounded-[28px] border border-white/5 bg-white/2 flex items-center justify-center text-cyan-400 group relative">
            <svg width="28" height="28" md:width="36" md:height="36" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"></path></svg>
        ...
        <div className="flex items-center gap-4 md:gap-8 w-full md:w-auto">
          <Link to="/admin/dashboard" className="flex-1 md:flex-none px-8 md:px-12 py-4 md:py-6 rounded-[20px] md:rounded-[28px] border border-white/5 bg-white/2 font-display text-[9px] md:text-[11px] font-bold text-cyan-400 uppercase tracking-widest hover:bg-white/5 transition-all text-center italic">Exit Module</Link>
          <button
            type="button"
            onClick={() => {
            ...
            className="flex-1 md:flex-none px-8 md:px-12 py-4 md:py-6 rounded-[20px] md:rounded-[28px] border border-rose-500/20 bg-rose-500/5 font-display text-[9px] md:text-[11px] font-bold text-rose-400 uppercase tracking-widest hover:bg-rose-500/10 transition-all text-center italic"
          >
            Sign Out
          </button>
        </div>
      ...
      {status ? (
        <div className="admin-reveal mb-8 md:mb-12 overflow-hidden rounded-[24px] md:rounded-[32px] border border-cyan-400/20 bg-cyan-400/5 p-6 md:p-8 text-center shadow-2xl">
          <p className="font-display text-[9px] md:text-[11px] font-bold text-cyan-400 uppercase tracking-[0.5em] md:tracking-[0.6em] italic">{status.replace(/_/g, " ")}</p>
        </div>
      ) : null}
      ...
      <div className="grid gap-8 md:gap-12 grid-cols-1 lg:grid-cols-[420px_minmax(0,1fr)]">
        <aside className="admin-reveal space-y-8 md:space-y-12">
          <GlassCard className="p-8 md:p-12 shadow-2xl rounded-[32px] md:rounded-[48px]">
            <div className="mb-8 md:mb-12 flex items-center justify-between border-b border-white/5 pb-8 md:pb-10 text-left">
              ...
              <div className="h-1.5 w-1.5 md:h-2 md:w-2 rounded-full bg-cyan-400 shadow-[0_0_8px_#00d2ff]" />
            </div>
            ...
            {loading ? (
              <div className="flex flex-col items-center gap-6 py-16 md:py-24 text-[#333] font-display text-[10px] md:text-[11px] uppercase tracking-widest font-bold">
                <div className="h-8 w-8 md:h-10 md:w-10 rounded-full border-2 border-cyan-400/20 border-t-cyan-400 animate-spin shadow-[0_0_20px_rgba(0,210,255,0.2)]" />
                ...
                {items.map((item) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      >
                        <svg width="16" height="16" md:width="18" md:height="18" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0 1 16.138 21H7.862a2 2 0 0 1-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v3M4 7h16"></path></svg>
                      </button>
                    </div>
                  </div>
                ))}
            ...
            {config.allowCreate && (
              ...
              >
                <span className="relative z-10 uppercase">Initialize New Asset +</span>
              </button>
            )}
          </GlassCard>
        </aside>
        ...
        <section className="admin-reveal min-w-0">
          <GlassCard className="relative overflow-hidden p-8 md:p-24 shadow-[0_80px_200px_rgba(0,0,0,1)] text-left rounded-[32px] md:rounded-[80px]">
            <div className="absolute top-0 right-0 h-1.5 md:h-2 w-40 md:w-60 bg-cyan-500 opacity-60" />
                    ...
                    <article key={item._id} className="group relative rounded-[32px] md:rounded-[48px] border border-white/5 bg-white/2 p-6 md:p-12 transition-all hover:border-cyan-400/20 duration-700 shadow-xl">
                      <div className="mb-8 md:mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8">
                        <div className="space-y-3 md:space-y-4">
                          ...
                          <p className="text-[9px] md:text-[11px] text-[#444] font-display tracking-widest uppercase font-bold italic">{item.email}</p>
                        </div>
                      ...
                      <div className="mt-8 md:mt-12 flex justify-end border-t border-white/5 pt-8 md:pt-10">
                        <button
                          ...
                          className="px-6 md:px-10 py-3 md:py-4 rounded-xl md:rounded-2xl border border-rose-500/20 text-[9px] md:text-[10px] font-bold text-rose-500 uppercase tracking-widest hover:bg-rose-500/10 transition-all font-display italic"
                        >
                          Destroy Record <span>×</span>
                        </button>
                      </div>
                    </article>
                  ))}
              ...
              <form onSubmit={onSubmit} className="space-y-12 md:space-y-16">
                <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-white/5 pb-8 md:pb-12 gap-8">
                  <div className="text-left">
                    ...
                    <h2 className="text-3xl md:text-5xl font-bold text-white tracking-tighter leading-tight italic">
                ...
                <div className="grid gap-x-12 gap-y-8 md:gap-x-16 md:gap-y-12 md:grid-cols-2 text-left">
                  <div className="space-y-4 md:space-y-5">
                    <label className="text-[9px] md:text-[11px] font-bold text-slate-700 uppercase tracking-[0.3em] md:tracking-[0.4em] ml-2 italic">Unique Identifier (Slug)</label>
                  ...
                  {type === "seoMeta" ? (
                    ...
                    <div className="space-y-4 md:space-y-5">
                      <label className="text-[9px] md:text-[11px] font-bold text-slate-700 uppercase tracking-[0.3em] md:tracking-[0.4em] ml-2 italic">Record Category</label>
                  ...
                  <div className="space-y-12 animate-in fade-in duration-700">
                    ...
                    <div className="grid gap-10 md:grid-cols-2">
                      <div className="space-y-3">
                        ...
                        <textarea rows={4} value={form.summaryEn} onChange={onChange("summaryEn")} className="w-full rounded-[24px] border-2 border-white/5 bg-black/40 px-6 py-5 text-white outline-none focus:border-cyan-400/40 resize-none" />
                      </div>
                      <div className="space-y-3">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.3em] ml-1">Brief_Abstract (BN)</label>
                        <textarea rows={4} value={form.summaryBn} onChange={onChange("summaryBn")} className="w-full rounded-[24px] border-2 border-white/5 bg-black/40 px-6 py-5 text-white outline-none focus:border-cyan-400/40 resize-none" />
                      </div>
                    </div>
                    ...
                    <div className="grid gap-10 md:grid-cols-3 bg-white/2 p-10 rounded-[40px] border border-white/5">
                      ...
                      <div className="space-y-3">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.3em] ml-1">Quality_Factor (1-5)</label>
                ...
                <div className="space-y-12 pt-12 border-t border-white/10 relative">
                  ...
                  <div className="grid gap-10 md:grid-cols-2">
                    <div className="space-y-3">
                      ...
                      <input value={form.seoTitleEn} onChange={onChange("seoTitleEn")} className="w-full rounded-[20px] border-2 border-white/5 bg-black/40 px-6 py-5 text-white outline-none focus:border-cyan-400/40" />
                    </div>
                    <div className="space-y-3">
                      <label className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.3em] ml-1">Metadata_Title (BN)</label>
                      <input value={form.seoTitleBn} onChange={onChange("seoTitleBn")} className="w-full rounded-[20px] border-2 border-white/5 bg-black/40 px-6 py-5 text-white outline-none focus:border-cyan-400/40" />
                    </div>
                  </div>
                  <div className="space-y-3">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-10 border-t border-white/10 pt-16">
                   <div className="flex flex-col sm:flex-row gap-6 w-full md:w-auto">
                    ...
                    {selectedId && (
                      <button type="button" onClick={() => onDeleteItem(selectedId)} disabled={saving} className="px-8 md:px-10 py-5 md:py-6 rounded-[20px] md:rounded-[24px] border-2 border-rose-500/20 bg-rose-500/5 font-mono text-[9px] md:text-[10px] font-bold text-rose-500 uppercase tracking-[0.3em] hover:bg-rose-500/10 transition-all italic">
                        ERASE_Artifact
                      </button>
                    )}
                  </div>
      ...
      <footer className="admin-reveal mt-16 md:mt-32 border-t border-white/5 pt-12 md:pt-16 flex flex-col md:flex-row justify-between items-center gap-8 md:gap-12 px-6 md:px-12 font-display text-[9px] md:text-[11px] text-[#222] uppercase tracking-[0.5em] md:tracking-[0.7em] font-bold italic text-center md:text-left">
         ...
         <span className="opacity-40 whitespace-nowrap">© 2026 Admin Execution Hub</span>
      </footer>
    </div>
  );
}
