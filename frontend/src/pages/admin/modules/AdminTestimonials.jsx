...
export default function AdminTestimonials() {
  ...
  const inputClasses = "w-full bg-white border border-slate-200 rounded-2xl px-7 py-5 text-slate-900 outline-none focus:border-cyan-400/50 focus:bg-white/[0.05] transition-all font-medium italic placeholder:text-slate-400 shadow-inner text-sm";
  const labelClasses = "flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.4em] text-slate-700 italic ml-4 mb-4";
      ...
      <div className="grid lg:grid-cols-[380px_1fr] gap-16">
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <div className="flex items-center justify-between mb-2">
                    <span className={`text-[8px] font-black uppercase tracking-[0.2em] italic ${selectedId === item._id ? 'text-sky-600' : 'text-slate-800'}`}>{item.category}</span>
                    <div className="flex gap-1.5">
                       {item.isFeatured && <Crown size={10} className="text-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.4)]" />}
                       <div className="h-2 w-2 rounded-full bg-emerald-500 shadow-[0_0_10px_#10b981]" />
                    </div>
                  </div>
                  <p className="text-sm font-black text-slate-900 italic truncate tracking-tight mb-1 uppercase">{item.title?.en || "Client Name"}</p>
                  <div className="flex items-center gap-3 mt-1.5">
                    <span className="text-[7px] font-black uppercase text-slate-900 tracking-widest bg-slate-50 px-2 py-0.5 rounded truncate">ROLE: {item.summary?.en || "N/A"}</span>
                    <span className="text-[7px] font-black uppercase text-slate-900 tracking-widest bg-slate-50 px-2 py-0.5 rounded">ID: {item._id?.slice(-4) || '....'}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="space-y-16 animate-in fade-in slide-in-from-right-4 duration-700">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 border-b border-slate-200 pb-12">
            <div>
               <h3 className="text-3xl font-black text-slate-900 tracking-tighter uppercase mb-2">{selectedId ? "Edit Testimonial" : "New Testimonial"}</h3>
               <p className="text-[10px] text-slate-700 font-bold uppercase tracking-widest italic">{selectedId ? `ID: ${selectedId}` : "Create a new professional endorsement"}</p>
            </div>
            ...
            <div className="flex items-center gap-6 px-10 py-5 bg-slate-50 border border-slate-200 rounded-3xl group/featured">
               <div className="flex items-center gap-3">
                  ...
                  <span className={`text-[9px] font-black uppercase tracking-[0.3em] italic ${form.isFeatured ? 'text-yellow-500' : 'text-slate-800'}`}>Featured Status</span>
               </div>
          ...
          {/* Media Module */}
          <div className="pt-8">
             <div className="bg-slate-50 border border-slate-200 rounded-[48px] p-12 relative overflow-hidden group/media">
                ...
                <div className="flex items-center gap-4 mb-10">
                   ...
                   <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-slate-700">Client Portrait</h3>
                </div>
